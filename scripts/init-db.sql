...
DROP TABLE IF EXISTS potential_properties CASCADE;
...
-- Potential Properties for ROI Analysis
CREATE TABLE potential_properties (
  ...
  estimated_monthly_rent DECIMAL(10, 2) NOT NULL,
  property_tax_annual DECIMAL(10, 2) DEFAULT 0.00,
  insurance_annual DECIMAL(10, 2) DEFAULT 0.00,
  hoa_monthly DECIMAL(10, 2) DEFAULT 0.00,
  property_management_percent DECIMAL(5, 2) DEFAULT 10.00,
  maintenance_monthly DECIMAL(10, 2) DEFAULT 0.00,
  other_expenses_monthly DECIMAL(10, 2) DEFAULT 0.00,
  notes TEXT,
  status VARCHAR(50) DEFAULT 'analyzing', -- 'analyzing', 'interested', 'offer_made', 'passed', 'purchased'
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Add column comments for potential properties
COMMENT ON COLUMN potential_properties.down_payment_percent IS 'Down payment as percentage of purchase price';
COMMENT ON COLUMN potential_properties.interest_rate IS 'Mortgage interest rate as percentage';
COMMENT ON COLUMN potential_properties.loan_term_years IS 'Mortgage term in years (typically 15 or 30)';
COMMENT ON COLUMN potential_properties.status IS 'Current status of this potential property';
...
CREATE INDEX idx_potential_properties_status ON potential_properties(status);
...
ALTER TABLE potential_properties 
ADD COLUMN IF NOT EXISTS is_cash_purchase BOOLEAN DEFAULT FALSE;

-- Add comment for documentation
COMMENT ON COLUMN potential_properties.is_cash_purchase IS 'TRUE for cash purchases (no financing), FALSE for financed purchases';

-- Optional: Update existing records where down_payment_percent = 100 to be marked as cash
UPDATE potential_properties 
SET is_cash_purchase = TRUE 
WHERE down_payment_percent = 100;